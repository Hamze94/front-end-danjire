...
const ReceiptPage = (props) => {
    ...
    const order = location.state?.order; // Access order from props
    console.log(order)
    useEffect(() => {
        const generatePDF = async () => {
            const receipt = receiptRef.current;
            const canvas = await html2canvas(receipt, { scale: 2 }); // Increase scale for better quality
            const imageData = canvas.toDataURL('image/png');

            const pdf = new jsPDF('p', 'pt', 'a5');
            const imgWidth = 1190.7; // A4 width in points
            ...
            <div className="flex h-screen w-full items-center justify-center bg-gray-600">
                <div className="w-80 rounded bg-gray-50 px-6 pt-8 shadow-lg">
                    ...
                    <div className="flex flex-col justify-center items-center gap-2">
                        <h4 className="font-semibold">{order.businessName}</h4>
                        <p className="text-xs">{order.businessAddress}</p>
                    </div>
                            ...
                            <tbody>
                                {order.products.map((product, index) => (
                                    <tr className="flex" key={index}>
                                        ...
                                        <td className="min-w-[44px]">${product.sellingPrice * product.quantity}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
